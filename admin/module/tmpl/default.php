  ...
  if ($storage_source != '') { // if record id or filepathname
    if (is_int($storage_source)) { //it is record id
      if ($code_stored != null) {
        ...
        echo '<div style="color:#FF0000;background:#FFFF00;">' . Text::sprintf('ERROR_RECORD', $storage_source) . '</div>';
      }
    } else { //it is file
      if (is_readable($storage_source)) {
        ...
        echo '<div style="color:#FF0000;background:#FFFF00;">' . Text::sprintf('ERROR_FILE', $storage_source) . '</div>';
      }
    }
  }
} else { //nothing to show
  echo '<div style="color:#FF0000;background:#FFFF00;">' . Text::sprintf('ERROR_CONTENT') . '</div>';
}
